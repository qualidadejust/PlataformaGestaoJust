// Autorização do JustAtestados. O Core é a fronteira real e o gateway já valida o JWT;
// aqui revalidamos o token para aplicar as PERMISSÕES do módulo (apontador × RH).
...
/** Nome de quem está logado (para snapshot/auditoria) — best-effort. */
export function atorNome(req: Request): string | undefined {
  const u = (req as any).user as TokenPayload | undefined;
  return u?.sub ? u.sub : undefined;
}
