// Porta de entrada: bloqueia o app até o login. Se a senha é temporária, força a troca.
...
function Botao({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...rest}
      className="w-full rounded-lg bg-sky-600 px-4 py-2 font-medium text-white transition hover:bg-sky-700 disabled:opacity-60 dark:bg-sky-500 dark:hover:bg-sky-600"
    >
      {children}
    </button>
  );
    ...
    return (
      <Cartao titulo="Entre com suas credenciais">
        <form onSubmit={onSubmit} className="space-y-4">
          <Campo label="E-mail" type="email" autoComplete="username" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <Campo label="Senha" type="password" autoComplete="current-password" value={senha} onChange={(e) => setSenha(e.target.value)} required />
          {erro && <Erro msg={erro} />}
