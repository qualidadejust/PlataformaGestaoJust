...
import { ArrowLeft, Filter, Search, SlidersHorizontal, AlertCircle, Save, CheckCircle2, MessageSquare, X, AlertTriangle, MessageCircle, Target, CalendarDays } from "lucide-react";
...
type CalibEntry = { potential: string; justification: string; consensus_date: string };
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-sm font-bold text-slate-900">Observações do Consenso</h3>
        ...
        <div className="p-6">
          <textarea
            ...
            placeholder="Registre o contexto do consenso, decisões tomadas, ajustes entre autoavaliação e avaliação do gestor..."
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-emerald-600" /> Consenso Finalizado
          </h3>
          ...
          {withoutPdi.length > 0 ? (
            ...
            <>
              <p className="text-sm text-slate-600 mb-6">
                Todos os colaboradores já têm PDI neste ciclo.
              </p>
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50">
                  Fechar
...
export function CalibrationView({ onNavigate }: { onNavigate?: (view: string, id?: string) => void }) {
    ...
    if (savedCalibrations.length > 0) {
      ...
      savedCalibrations.forEach(c => {
        initial[c.employee_id] = {
          ...
          consensus_date: c.consensus_date ?? '',
        };
      });
    ...
    if (existing) {
      ...
      if (!calibrations[ev.employee_id]) {
        setCalibrations(prev => ({
          ...prev,
          [ev.employee_id]: { potential: '', justification: '', consensus_date: '' },
        }));
      }
    }
    return acc;
  ...
  function buildEntries() {
    return Object.entries(calibrations).map(([employee_id, c]) => ({
      ...
      consensus_date: c.consensus_date || null,
    }));
  }
  ...
  function getGroupScores(group: EmployeeGroup) {
    ...
    return { selfScore, managerScore, delta, hasDiscrepancy };
  }
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-600 mt-2 max-w-2xl">
            Agende e registre o consenso de cada colaborador. Compare as notas de autoavaliação e gestor, defina o potencial e agende a reunião.
          </p>
        </div>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-3">
            ...
            >
              <CheckCircle2 className="w-4 h-4" /> Finalizar Consenso
            </button>
          </div>
          {finalizError && (
      ...
      <div className="bg-brand-50 border border-brand-100 rounded-2xl p-5 mb-8 flex items-start gap-4">
        ...
        <div>
          <h3 className="text-sm font-bold text-brand-900 mb-1">
            Diretrizes do Consenso {selectedCycle ? `— ${selectedCycle.name}` : ''}
          </h3>
          <ul className="text-sm text-brand-800 space-y-1 list-disc pl-4">
            <li>Discrepâncias <strong>&gt; 1 ponto</strong> entre autoavaliação e gestor são destacadas em laranja.</li>
            <li>O consenso é feito <strong>por pessoa</strong>: agende a reunião, discuta as notas e registre o potencial definido.</li>
            <li>Potencial considera o horizonte de sucessão: <strong>Consolida</strong> (função atual), <strong>Desenvolve</strong> (1–2 anos), <strong>Acelera</strong> (sucessão imediata).</li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        {!selectedCycleId ? (
          <div className="p-12 text-center text-slate-400">
            ...
            <p className="font-medium">Selecione um ciclo para iniciar.</p>
          </div>
        ) : isLoading ? (
          ...
          <div className="p-12 text-center text-slate-400">
            ...
            <p className="text-sm mt-1">Crie avaliações em "Avaliações" e preencha-as para iniciar o consenso.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-xs text-slate-500 uppercase tracking-wider bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 font-bold bg-brand-50/50 text-brand-800 border-x border-brand-100/50">Data do Consenso</th>
                  <th className="px-6 py-4 font-bold bg-brand-50/50 text-brand-800">Potencial</th>
                  <th className="px-6 py-4 font-bold text-center">Observações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filtered.map((group) => {
                  const calib = calibrations[group.employee_id] ?? { potential: '', justification: '', consensus_date: '' };
                  const initials = group.employee_name.split(' ').filter(Boolean).map(n => n[0]).slice(0, 2).join('').toUpperCase();
                  const { delta, hasDiscrepancy } = getGroupScores(group);
                    ...
                    <tr key={group.employee_id} className={cn("hover:bg-slate-50 transition-colors group", hasDiscrepancy && "bg-amber-50/30")}>
                      ...
                      <td className="px-6 py-5 whitespace-nowrap bg-brand-50/30 border-x border-brand-50/50">
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            type="date"
                            value={calib.consensus_date}
                            onChange={e => setCalib(group.employee_id, 'consensus_date', e.target.value)}
                            className="px-2 py-1 bg-white border border-brand-200 rounded-lg text-xs font-medium text-brand-900 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5 whitespace-nowrap text-center">
                        <button
                          ...
                          title={calib.justification || 'Adicionar observações'}
                          className={cn(
